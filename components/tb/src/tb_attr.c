...
static esp_err_t tb_attr_unsubscribe_from_request(const thingsboard *tb, esp_event_handler_t request_attributes_handler);
...
static void tb_attr_subscribe_to_shared_handler(void *event_handler_arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
    ...
    case MQTT_EVENT_DATA:
    {
        ...
        err = esp_event_post_to(tb->event_loop, TB_EVENTS, TB_EVENT_SHARED_ATTRIBUTE, event->data, event->data_len, TB_UTIL_TIMEOUT_TICKS);
        if (err != ESP_OK)
        {
            goto cleanup_data;
        }
...
static void tb_attr_request_shared_handler(void *event_handler_arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
    tb_attr_request_handler("shared", TB_EVENT_SHARED_ATTRIBUTE, event_handler_arg, event_base, event_id, event_data);
}

static void tb_attr_request_client_handler(void *event_handler_arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
    tb_attr_request_handler("client", TB_EVENT_CLIENT_ATTRIBUTE, event_handler_arg, event_base, event_id, event_data);
}
