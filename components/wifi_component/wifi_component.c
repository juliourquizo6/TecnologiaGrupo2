...
esp_err_t save_thingsboard_url(const char *url) {
    ...
    err = nvs_set_str(nvs_handle, "thingsboard", url);
...
esp_err_t load_thingsboard_url(char *url, size_t max_len) {
    ...
    err = nvs_get_str(nvs_handle, "thingsboard", NULL, &required_size);
    if (err == ESP_ERR_NVS_NOT_FOUND) {
        ...
        if (required_size > max_len) {
            ...
            nvs_get_str(nvs_handle, "thingsboard", url, &required_size);
        }
    }
    nvs_close(nvs_handle);
    return err;
