...
static EventGroupHandle_t timers_event_group;
...
void set_timers(void *arg)
{
    ...
    xEventGroupSetBits(timers_event_group, TIMERS_READY_BIT);
}
...
static void sgp30_co2_init(void)
{
    ...
    timers_event_group = xEventGroupCreate();
    timers_init();
    ...
    ESP_ERROR_CHECK(esp_timer_start_once(start_timer, 15000001));
}
...
{
    xEventGroupWaitBits(timers_event_group, TIMERS_READY_BIT, pdFALSE, pdTRUE, portMAX_DELAY);
    ESP_ERROR_CHECK(esp_timer_restart(tm_lectura_handle, frec * 1000));
}

void sensor_co2_set_frecuencia_envio(int frec)
{
    xEventGroupWaitBits(timers_event_group, TIMERS_READY_BIT, pdFALSE, pdTRUE, portMAX_DELAY);
    ESP_ERROR_CHECK(esp_timer_restart(tm_envio_handle, frec * 1000));
}
